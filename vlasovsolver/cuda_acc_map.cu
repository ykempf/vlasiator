...
bool accelerateVelocityMeshCuda(Realf **blockDatas, vmesh::GlobalID **blockIDs, vmesh::LocalID *nBlocks,const  vmesh::LocalID gridLength[3], const Real blockSize[3], const uint nCells ){
   ...
   for (int i = 0; i < nCells; i++) {
      ...
      vmesh::VelocityMeshCuda<vmesh::GlobalID, vmesh::LocalID>* h_vmesh;
      
      vmesh::createVelocityMeshCuda(&d_vmesh, &h_vmesh, blockDatas[i], blockIDs[i], nBlocks[i], gridLength, blockSize_f, streams[i]);

      vmesh::sortVelocityBlocks(d_vmesh, h_vmesh, 0, streams[i]);
      vmesh::sortVelocityBlocks(d_vmesh, h_vmesh, 1, streams[i]);
      vmesh::sortVelocityBlocks(d_vmesh, h_vmesh, 2, streams[i]);

      vmesh::destroyVelocityMeshCuda(d_vmesh, h_vmesh, streams[i]);
   }
   for (int i = 0; i < nCells; i++) {
